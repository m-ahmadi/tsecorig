// Decompiled with JetBrains decompiler
// Type: TseClient.ColumnsFileReadResult
// Assembly: TseClient, Version=2.0.16.0, Culture=neutral, PublicKeyToken=null
// MVID: 55DBD486-C597-460B-B54A-EAFCAFB4B891
// Assembly location: D:\Program Files\TseClient 2.0\TseClient.exe

namespace TseClient
{
  public class ColumnsFileReadResult
  {
    public System.Collections.Generic.List<ColumnInfo> List;
    public FileReadResult ReadResult;
  }
}
